...
async function main() {
  ...
  // Quiz for 1.1
  const q1_1_1 = await prisma.quizQuestion.create({
    data: {
      topicId: sub1_1.id,
  ...
  const q1_1_2 = await prisma.quizQuestion.create({
    data: {
      topicId: sub1_1.id,
...
main()
  ...
  .finally(async () => {
    // We don't need prisma.$disconnect() since we are using driver adapter, but closing is fine
  });
